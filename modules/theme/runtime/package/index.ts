 ...
 */
function derivePaletteProperties(
  theme: Theme,
  { tones, prefix }: { tones: number[]; prefix?: string }
): Properties {
  const properties = {} as Record<string, string>
  for (const [key, value] of Object.entries(theme.palettes)) {
    for (const tone of tones) {
      properties[`--${prefix ?? ''}${tokenize(key)}${tone}`] = hexFromArgb(
        value.tone(tone)
      )
    }
  }
 ...
 */
function deriveSchemeProperties(
  scheme: Scheme,
  ...
  for (const [key, value] of entries) {
    properties[`--${prefix ?? ''}${tokenize(key)}${suffix ?? ''}`] =
      hexFromArgb(value)
  }
  return properties
}
 ...
 */
function deriveSurfaceElevationProperties(
  argb: number,
  options: {
    prefix?: string
    suffix?: string
  ...
  tones.forEach((tone, index) => {
    ...
    const token = `--${options?.prefix ?? ''}surface-level${index}${
      options?.suffix ?? ''
    }`
 ...
 */
function rgbProperties(
  properties: Record<string, string>,
  { separator, suffix }: { separator: ',' | ' '; suffix: string } = {
    separator: ',',
    suffix: '-rgb'
  }
) {
...
function paletteFromCustomColor(
  ...
  { tones, sourceColor }: { tones: number[]; sourceColor: number }
) {
...
function deriveCustomSchemeProperties(
...
const getSurfaceContainers = (
  ...
  return Object.entries(roleToneMap).reduce((acc, [role, tone]) => {
    ...
    const name = `--${options?.prefix ?? 'md-sys-color-'}${role}${
      options?.suffix ?? ''
    }`
    return {
      ...acc,
...
const getSurfaceTints = (
  ...
  const schemeNames = ['background-tint']
...
function deriveProperties(
  ...
  const ccSchemes = theme.customColors.reduce(
    (acc, color) => ({
      ...acc,
  ...
  const surfaceElevations = {
    ...deriveSurfaceElevationProperties(
      dark ? theme.schemes.dark.primary : theme.schemes.light.primary,
      { prefix: prefix?.color }
    ),
    ...(brightnessSuffix
      ? {
  ...
  const surfaceContainers = {
    ...getSurfaceContainers(theme.schemes[dark ? 'dark' : 'light'].surface, {
      dark,
      prefix: prefix?.color
    }),
    ...(brightnessSuffix
      ? {
          ...getSurfaceContainers(theme.schemes.light.surface, {
            suffix: '-light',
            prefix: prefix?.color
          }),
          ...getSurfaceContainers(theme.schemes.dark.surface, {
            dark: true,
            suffix: '-dark',
            prefix: prefix?.color
          })
...
export { propertiesFromTheme, hexAFromArgb, rgbFromHex, tokenize }
